...
export default function QuesitonsPartTow({ handleOptionSelected, selectedOptions, setSelectedAll}) {
  ...
  const chapter2 = individualExams?.slice(12, 24);

  // validate all questions has a selected answer
  const allQuestionsAnswered = chapter2.every((question) =>
  ...
  return (
    <div>
      <h1 className='text-right text-teal-800 text-lg'>Quesitons Part Two</h1>
      {chapter2.map((question: QuestionProps) => {
        return (
