...
export default function QuesitonsPartFour({ handleOptionSelected, selectedOptions, setSelectedAll }) {
  ...
  const chapter4 = individualExams?.slice(36, 50);

  // validate all questions has a selected answer
  const allQuestionsAnswered = chapter4.every((question) =>
  ...
  return (
    <div>
      <h1 className='text-right  text-teal-800 text-lg'>Quesitons Part Four</h1>
      {chapter4.map((question: QuestionProps) => {
        return (
